...
boss._listen = async function ()
{
    ...
    let file = boss._gen_file(boss.id)
...
boss._connect_worker = async function (worker)
{
    ...
    let file = boss._gen_file(worker.id)
    ...
    boss._init_conn(conn, worker)

    //等待本端登陆成功
    await awesome.wait(`llogin[${worker.id}]`)
}

boss._init_conn = function (conn, worker)
{
    let decoder = {
        wait: 0,
        in_buffers: [],
    ...
    conn.on("error", function (err)
    {
        ...
        if (worker)
        {
            worker.retry = ++(worker.retry || 0)
            console.log(`----------------------------reconnect,${worker.id}`)
...
boss.broad = function (func, ...args)
{
    for (let [id, worker] of data.workers)
    {
        if (id == boss.cluster)
        {
            continue
        }
        boss.send_worker(worker, func, ...args)
    }
}
