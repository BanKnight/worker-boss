...
me._init_cluster = function (conn, cluster)
{
    ...
    conn.on("error", function (err)
    {
        if (cluster)
        {
            ...
            if (cluster.retry > 3)
            {
                console.warn(`connect cluster ${cluster.id},err:${err},reconnecting`)
            }
...
me._send = function (cluster, ...args)
{
    ...
    let buffer = me._pack(cluster, "s", ...args)
...
me._call = async function (cluster, ...args)
{
    if (typeof cluster == "number")
    {
        cluster = data.clusters.get(cluster)
    ...
    let buffer = me._pack(cluster, "c", session, ...args)
...
me._after_login = async function (conn, buffer)
{
    ...
    {
        case "s":
            md_workers.send_worker(target, ...args)
            break
        case "c":
            {
                let [session, ...left_args] = args

                let ret = await md_workers.call_worker(target, ...left_args)
