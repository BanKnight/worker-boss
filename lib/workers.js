...
me.on_dispatch = function (cb)
{
    ...
    setImmediate(async () =>
    {
        ...
        for (let one of standby)
        {
            if (one[0] == "i")
            {
                me._on_invoke(one[1], ...one[2])
            }
            else
            {
                me._on_call(one[1], one[2], ...one[3])
            }
        }
    })
}
...
me._new_worker_vm = async function (worker, full_path, ...args)
{
    ...
    let vm_process = Object.assign({}, process) //浅拷贝
...
me._on_invoke = function (worker, ...args)
{
    if (data.standby)
    {
        data.standby.push(["i", worker, args])

        console.error(`invoke from worker[${worker.id}]: ${args},but no handler,please use me.on_dispatch`)
    }
    else
    {
        data.dispatch(...args)
...
me._on_call = async function (worker, session, ...args)
{
    if (data.standby)
    {
        data.standby.push(["i", worker, session, args])

        console.error(`call from worker[${worker.id}],but no handler,please use me.on_dispatch`)
    }
    else
    {
        let ret = await data.dispatch(...args)

        me._ret_worker(worker, session, ret)
    }
}
me._on_resp = function (worker, session, ret)
{
